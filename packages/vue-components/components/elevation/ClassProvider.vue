<template>
    <slot :shadow="[$style.surface, $style[`level-${props.level}`]]"></slot>
</template>

<script setup lang="ts">
import type { IElevation } from './Elevation.type'

const props = defineProps<IElevation>()
</script>
...
.surface {
    ...
    --_elevation-level: var(--mamv-elevation-level, var(--_current-elevation-level, 0));
    --_elevation-shadow-color: var(--mamv-elevation-shadow-color, var(--_current-elevation-shadow-color, #000000));
}
